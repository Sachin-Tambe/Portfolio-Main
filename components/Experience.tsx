...
// Simple helper to bold **text** in strings
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)]">
                <RevealOnScroll direction={index % 2 === 0 ? 'right' : 'left'} delay={index * 100}>
                  <div className="bg-surface p-6 rounded-xl border border-white/10 shadow-lg hover:border-primary/40 transition-all duration-300 hover:shadow-primary/10 hover:-translate-y-1">
                    ...
                    <div className="text-secondary font-medium mb-1 text-sm">{exp.company}</div>
                    <p className="text-gray-400 text-xs mb-4 italic">Focused on {exp.description.toLowerCase()}</p>
                    <ul className="space-y-2">
                      {exp.achievements.map((achievement, i) => (
                        <li key={i} className="text-gray-300 text-sm flex items-start">
                          <span className="mr-2 text-primary">•</span>
                          <span>{formatWithBold(achievement)}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
